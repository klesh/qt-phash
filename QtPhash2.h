...
namespace QtPhash {
  ...
  int computeDistance(const quint64 hash1,const quint64 hash2){
    ...
    const quint64 m1  = 0x5555555555555555ULL; // 0x55 : 0b 0101 0101
    const quint64 m2  = 0x3333333333333333ULL; // 0x53 : 0b 0011 0011
    const quint64 h01 = 0x0101010101010101ULL; // 0x01 : 0b 0000 0001
    const quint64 m4  = 0x0f0f0f0f0f0f0f0fULL; // 0x0f : 0b 0000 1111
    x -= (x >> 1) & m1;
    x = (x & m2) + ((x >> 2) & m2);
    x = (x + (x >> 4)) & m4;
    return (x * h01)>>56;
